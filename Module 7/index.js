...
const VendorMetricsIntentHandler = {
	canHandle(handlerInput) {
		const request = handlerInput.requestEnvelope.request;
		return request.type === 'IntentRequest'
      && request.intent.name === 'VendorMetricsIntent';
	},
	async handle(handlerInput) {
		const filledSlots = handlerInput.requestEnvelope.request.intent.slots;
		const slotValues = getSlotValues(filledSlots);
		...
		try {
			...
			if (response.length > 0) {
				console.log('GETTING LIST OF ALL VENDOR NAMES');
				//GET LIST OF ALL DEPARTMENT NAMES
				var all_vendors = [];
				response.forEach(function(record, i) {
					all_vendors.push(record['vendor']);
				});

				//FILTER TO UNIQUE DEPARTMENT NAMES
				var unique_vendors = all_vendors.filter( onlyUnique );
				console.log(unique_vendors.length);

				//CREATE AN ARRAY CONTAINING THE DEPARTMMENTS, AND TOTAL AMOUNT SPENT BY EACH UNIQUE DEPARTMENT
				var checkbook = [];

				unique_vendors.forEach(function(vendor, i) {
					var amount_spent = 0;
					response.forEach(function(record, i) {
					...
					checkbook.push({vendor,amount_spent});
				});
				...
				console.log('Most money spent on ' + highest_spend['vendor'] + ' amounting to ' + highest_spend['amount_spent']);
				console.log('Least money spent on ' + least_spend['vendor'] + ' amounting to ' + least_spend['amount_spent']);

				if (slotValues.amount_type.resolved === 'most') {
					speechText = `So in ${slotValues.year.resolved}, the most money was spent in the ${highest_spend['vendor']}, with a total of $${highest_spend['amount_spent']}. What other year would you like to know about?`;
				} else {
					speechText = `So in ${slotValues.year.resolved}, the least money was spent in the ${least_spend['vendor']} vendor, with a total of $${least_spend['amount_spent']}. What other year would you like to know about?`;
				}
			} else {
				speechText = `I am sorry I could not find any info for spending in ${slotValues.year.resolved}.`;
			}
...
const DepartmentActivityMetricsIntentHandler = {
	canHandle(handlerInput) {
		const request = handlerInput.requestEnvelope.request;
		return request.type === 'IntentRequest'
      && request.intent.name === 'DepartmentActivityMetricsIntent';
	},
	async handle(handlerInput) {
		const filledSlots = handlerInput.requestEnvelope.request.intent.slots;
		const slotValues = getSlotValues(filledSlots);
		...
		try {
			...
			debugLog('response.length',response.length);
				...
				if (response.length > 0) {
				console.log('GETTING LIST OF ALL ACTIVITY NAMES');
				//GET LIST OF ALL DEPARTMENT NAMES
				var all_expenditure_types = [];
				response.forEach(function(record, i) {
					all_expenditure_types.push(record['expenditure_activity_type']);
				});

				//FILTER TO UNIQUE DEPARTMENT NAMES
				var unique_expenditure_types = all_expenditure_types.filter( onlyUnique );
				console.log(unique_expenditure_types.length);

				//CREATE AN ARRAY CONTAINING THE DEPARTMMENTS, AND TOTAL AMOUNT SPENT BY EACH UNIQUE DEPARTMENT
				var checkbook = [];

				unique_expenditure_types.forEach(function(expenditure_activity_type, i) {
					var amount_spent = 0;
					response.forEach(function(record, i) {
						if (record['expenditure_activity_type'] === expenditure_activity_type){
							amount_spent = amount_spent + parseInt(record['transaction_amount']);
						}
					});
					checkbook.push({expenditure_activity_type,amount_spent});
				});
				...
				console.log('Most money spent on ' + highest_spend['expenditure_activity_type'] + ' amounting to ' + highest_spend['amount_spent']);
				console.log('Least money spent on ' + least_spend['expenditure_activity_type'] + ' amounting to ' + least_spend['amount_spent']);

				if (slotValues.amount_type.resolved === 'most') {
					speechText = `In ${slotValues.year.resolved}, ${slotValues.department.resolved} spent the most money in the ${highest_spend['expenditure_activity_type']} activity, with a total of $${highest_spend['amount_spent']}. What else would you like to know about?`;
				} else {
					speechText = `In ${slotValues.year.resolved}, ${slotValues.department.resolved} spent the most money in the ${least_spend['expenditure_activity_type']} activity, with a total of $${least_spend['amount_spent']}. What else would you like to know about?`;
				}
			} else {
				speechText = `I am sorry I could not find any info for spending in ${slotValues.year.resolved}.`;
			}
...
exports.handler = Alexa.SkillBuilders.custom()
	.addRequestHandlers(
		...
		VendorMetricsIntentHandler,
		DepartmentActivityMetricsIntentHandler,
		RepeatIntentHandler,
		HelpIntentHandler,
